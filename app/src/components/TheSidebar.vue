<script setup>
    ...
    // // const updateMode = (status) =>{
    // //     console.log("darkMode:", status)
    // //     window.localStorage.setItem('darkMode', status);
    // //     console.log(window.localStorage.getItem('darkMode'))
    // // }
    
    // // let getModeStatus = window.localStorage.getItem('darkMode')
</script>

<template>
    <aside id = "body" class="flex flex-col items-center md:items-stretch space-y-2 md:space-y-4">
        <div class="flex flex-col items-center md:items-stretch space-y-2">
            <router-link :to="{ name: 'Home' }" class="rounded-full p-3 md:w-full inline-flex items-center space-x-4">
                ...
                <div class="text-xl hidden md:block"><img :src="require('../assets/logo.png')" :style="{height: 50 +'px'}"/></div>
            </router-link>
